...
const char *messageData = "{\"messageId\":%d, \"deviceId\":\"bme280B\", \"Temperature\":%.2f, \"Humidity\":%.2f}";
...
{
    Serial.println("Start demo function");
    int result = 0;
    ...
    do
    {
        ...
        snprintf(messagePayload,MESSAGE_MAX_LEN, messageData, message_id, temp_event.temperature, humidity_event.relative_humidity);
        // Construct the iothub message from a string or a byte array
        //message_handle = IoTHubMessage_CreateFromString(telemetry_msg); - original
        //message_handle = IoTHubMessage_CreateFromString(messagePayload);
        message_handle = IoTHubMessage_CreateFromByteArray((const unsigned char*)messagePayload, strlen(messagePayload));
...
void setup() {
   ...
   IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol = HTTP_Protocol;
    ...
    Serial.println("Creating IotHub connection");
    ...
    if (device_ll_handle == NULL)
    {
        ...
        Serial.println("Check connection string");
    }
    else
    {
        ...
        int diag_off = 0;
        Serial.println("IoThub connection success");
